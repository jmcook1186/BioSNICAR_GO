...
% and is only called if the driver is set to "GO" for the calculation of
% ice optical properties, otherwise an equivalent script is used that
% calculates optical properties using Mie theory.
%
%
%%%%%%%%%%  Input parameters: %%%%%%%%%%%
...
function data_out = snicar8d_GO(BND_TYP_IN, DIRECT_IN, APRX_TYP_IN, ...
                             ...
                             rho_snw_in, side_length_in, depth_in, nbr_aer_in, ...
...
wvl     = ncread(strcat(wrkdir,'ice_geom_20000_20000.nc'),'wvl').*10^6;
nbr_wvl = length(wvl);
...
if (BND_TYP==1)
    ...
    if (DIRECT == 1)
        ...
        load ./GO_files/mlw_sfc_flx_frc_clr.txt;
    ...
    elseif (DIRECT == 0)
        ...
        load ./GO_files/mlw_sfc_flx_frc_cld.txt;
...
for n=1:nbr_lyr
    ...
    fl_in = strcat(wrkdir,fl_stb1,int2str(side_length(n)),'_',int2str(depth(n)),fl_stb2);
