% BioSNICAR_GO written by Joseph Cook (joe.cook@sheffield.ac.uk) as a
% version of the BiOSNICAR model (Cook et al., 2017, TC) that uses ice
% optical properties calculated using geometric optics instead of Mie
% scattering theory, enabling application to arbitrarily large ice grains.
% BioSNICAR and BioSNICAR_GO are based upon the original SNICAR code
% written by Mark Flanner (flanner@umich.edu).
%
...
%
% The function reads in optical properties from external NetCDF
% files based on defined properties of each layer. These NetCDF files are
% generated by running the python script Geometric_Optics.py provided in
% the BioSNICAR_GO repository. In the initial release file sare provided
% for ice grains between 5000 - 30000 microns in both side length and
% column depth at a resolution of 1000 microns. Additional files for any 
% dimensions can easily be created using the python script.
%
...
% NOTE: The geometric optics are based on a parameterisation provided by
% Van Diedenhoven 2014 Journal of the Atmospheric Sciences including the
% corrections noted int he published corrigendum

% NOTE: In this release I have removed the option to run this as a
% standalone script. BioSNICAR_GO must be called from the driver script.

% NOTE: The path to the working directory in line 119 in this script will
% need to be updated for your system.
...
fl_in8 = strcat(wrkdir,fl_snw_alg); % JC EDIT
