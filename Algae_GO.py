...
def calc_optical_params(r, depth, reals, imags, wavelengths, plots=False, report_dims=False, ThreeBand=False):
    ...
    if plots:
        plt.figure(1)
        plt.plot(wavelengths, SSA_list, label='{}x{}'.format(r, depth)), plt.ylabel('SSA'), plt.xlabel(
            'Wavelength (um)'), plt.grid(False), plt.legend(loc='best', ncol=2)
        plt.figure(2)
        plt.plot(wavelengths, Assy_list, label='{}x{}'.format(r, depth)), plt.ylabel('Assymetry Parameter'), plt.xlabel(
            'Wavelength (um)'), plt.grid(False), plt.legend(loc='best', ncol=2)
        plt.figure(3)
        plt.plot(wavelengths, absXS_list, label='{}x{}'.format(r, depth)), plt.ylabel(
        ...
        plt.plot(wavelengths, X_list, label='{}x{}'.format(r, depth)), plt.ylabel('Size Parameter X'), plt.xlabel(
            'Wavelength (um)'), plt.grid(False), plt.legend(loc='best', ncol=2)
        plt.figure(5)
        plt.plot(wavelengths, MAC, label='{}x{}'.format(r, depth)), plt.ylabel('MAC'), plt.xlabel(
            'Wavelength (um)'), plt.grid(False), plt.legend(loc='best', ncol=2)
    ...
    return Assy_list, SSA_list, absXS_list, MAC, depth, r, Chi_abs_list, Reff, X_list, ThreeBandabsXS,ThreeBandAssy, ThreeBandChi, ThreeBandSSA, ThreeBandX


def net_cdf_updater(filepath,filepath3Band, Assy_list, SSA_list, MAC, depth, r, density, ThreeBand = False):

    if ThreeBand:
        Band1MAC = np.mean(MAC[0:49])
        Band2MAC = np.mean(MAC[50:119])
        Band3MAC = np.mean(MAC[120:249])
        ...
        algfile['asm_prm'] = np.squeeze(ThreeBandAssy)
        algfile['ss_alb'] = np.squeeze(ThreeBandSSA)
        algfile['ext_cff_mss'] = ThreeBandMAC
        ...
        algfile.to_netcdf(str(filepathIN + 'ThreeBand_algae_geom_{}_{}.nc'.format(str(r), str(depth))))
...
#            reals, imags, MAC, wavelengths = preprocess_RI()
#            Assy_list,SSA_list,absXS_list,MAC_list,depth,r,Chi_abs_list,Reff,X_list,ThreeBandabsXS,ThreeBandAssy, \
#            ThreeBandChi, ThreeBandSSA, ThreeBandX = calc_optical_params(r,depth,reals,imags,wavelengths,plots=False,
#                                                                         report_dims = True, ThreeBand=True)
#            net_cdf_updater(filepath,filepath3Band,Assy_list,SSA_list,MAC_list,depth,r,1400, ThreeBand=True)